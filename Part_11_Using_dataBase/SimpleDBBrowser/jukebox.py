...
class DataListBox(ScrollBox):
    ...
    def link(self, widget, link_filed):
        self.linked_box = widget
        widget.link_filed = link_filed

    def requery(self, link_value=None):
        self.link_value = link_value
        if link_value and self.link_filed:
            sql = self.sql_select + " WHERE " + self.link_filed + "=?" + self.sql_sort
    ...
    def on_select(self, event):
        if self.linked_box:
            ...
            if self.link_value:
                ...
                sql_where = " WHERE " + self.field + "= ? AND " + self.link_filed + "=?"
            else:
                sql_where = " WHERE " + self.field + "= ?"

            value_id = self.cursor.execute(self.sql_select + sql_where, value).fetchone()[1]
            self.linked_box.requery(value_id)
